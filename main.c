...
/**
 * 메인 시뮬레이션 루프
 * 고정된 타임스텝으로 실행되며 프레임 속도를 60Hz로 유지
 */
void cycle(void) {
    ...
    int count = 0;       // 프레임 카운터 - 디버깅 및 종료 조건용
    ...
    while (!quit) {
        ...
        // 다음 틱까지 남은 시간 계산
        timespec_t rem; // rem: remaining time (남은 시간)
        rem = ts_sub(&now, &next_tick);
