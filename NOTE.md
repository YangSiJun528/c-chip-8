...
## 12. 터미널 기반 디스플레이 출력 구현

구현은 이미 예시 코드가 있어서 어렵진 않다고 생각하는데,
몇 프레임으로 보여줄지가 문제임.

loop마다 하기에는 너무 과하고, 너무 느리면 디스플레이 변화를 못 보여줄 수도 있고.
GPT한테 물어보니까 대부분 타이머 시간인 60HZ 단위로 한다고 해서.
그렇게 설정함.

그리고 로깅 내용이 stdout으로 나오고 있었는데,
이러면 디스플레이 출력에 방해되니까 파일에 로깅하도록 함.

실제로 해보니 나쁘지 않은거 같음. 실제로 움직이는 프로그램을 띄워봐야 하긴 하겠지만...

코드가 점점 더러워지고 있다.  
거의 마지막 단계라 끼워넣는 식으로 만드는 것도 있음.  
다 완성하고 나면 리팩토링 좀 하기.

## 13. 사운드 처리 기능 구현

이건 어떻게 할 지 모르겠어서 GPT한테 물어보니까
터미널에선 간단하게 비프음 내는 방법을 소개해줘서 그걸 사용.

소리가 하나고 주파수도 알아서 하는거라 쉽게 만들 수 있었음.

chip8보다 조금만 더 복잡한거 만드려면 SDL 같은게 필수긴 할 듯.

## 14. Pong Rom으로 테스트

#### 간단한 수정

- `Fx33` 처리 case문에서 default 처리 잘못된 부분 수정
- `8xy4` 처리 중 오버플로우로 인해 carry 확인이 동작하지 않던 문제 수정
- `8xy7` 레지스터 값 할당이 잘못된 문제 수정
- `8xyE` 비트 처리가 잘못된 문제 수정
- `Fx0A` 신규 입력을 검사하는 조건이 잘못된 문제 수정
- `Fx33` 레지스터 값 대신 opcode의 값을 할당하던 문제 수정
- `get_key_index()` 대소문자 무시하고 검사하게 변경
- `Fx29` 폰트 바이트 값 알맞게 수정

#### 키 입력 관련 문제

Pong 게임에서 AI랑 공이랑 점수는 잘 바뀌는데, 내 키 입력이 아주 드물게 입력됨.
로깅 추가하고, LLM한테 물어보고 다 해봤는데 잘 안됨. 2시간동안 삽질중

어이없게도 keypad index 구하는 부분이 잘못되어 있었음.
이제 대충 기능에는 문제가 없어보임.
...
1. 게임 플레이 관련
    ...
    2. 화면이 번쩍거리는게 좀 있음
2. 기능 개선
    1. 여러 ROM을 동적으로(게임 선택 가능) 불러올 수 있으면 좋겠음.
    2. 게임 종료가 불가능함. esc키같이 키패드 외의 특정 키를 입력했을 때 종료시키기
3. 코드 관련
    1. 코드 분리가 잘 안되고 여러 기능이 한 함수에 묶여있음.
    2. 매직넘버나 어떤 연산인지 이해하기 어려운 동작이 있음.
        - 비트 연산같은건 MASK 값 만들어서 명확하게 할 수 있을듯?

## 16. 코드 개선 - 역할별로 코드 분리하기

- 어떤게 좋을지는 LLM 의견을 받았음.

여러 파일로 나뉘면서 고민이 생겼는데,

`#include`가 필요 없지만 `#include`를 명시적으로 표현하는게 적절한 경우에는 어떻게 하는게 맞는가?

Gemini 2.5 피셜
```
C에서 이런 문제를 해결하는 일반적인 접근 방식
C에서는 "최소한의 필요한 헤더만 포함한다"와 "직접 사용하는 심볼이 정의된 헤더를 명시적으로 포함한다"는 두 가지 원칙 사이에서 균형을 찾습니다.
원칙 1: 필요한 것만 포함 (Minimize Includes)
불필요한 헤더를 포함하면 컴파일 시간이 약간 늘어날 수 있고, 심볼 이름 충돌의 가능성도 미미하게나마 증가합니다.
또한, 어떤 모듈이 실제로 어떤 다른 모듈/기능에 의존하는지 파악하기 어려워질 수 있습니다.
원칙 2: 직접 의존성은 명시적으로 (Explicitly Include Direct Dependencies)
이것이 현재 논의에 더 부합하는 원칙입니다.
main.c가 FONTSET_ADDR 같은 chip8_config.h의 매크로를 직접 사용한다면, chip8_config.h를 #include하는 것이 좋습니다.
chip8_struct.h가 chip8_config.h를 포함하는 것은 chip8_t를 정의하기 위한 것이지, chip8_struct.h를 사용하는 모든 파일에게 chip8_config.h의 모든 내용을 제공하기 위함이 주 목적은 아닐 수 있습니다. 
```

결론: 가드 써서 중복 호출해도 상관없으니까 추상화해야하는 의존이 아니라 직접적으로 쓰이는 의존이라면 명시하는게 좋다.

## 17. 플젝 중단

- 파일 분리가 쉽지 않음.
    - 차라리 핵심 chip8 코드 뺴고 다 갈아 엎는게 빠를거 같음.
    - 처음부터 역할 생각해서 코드 나누고
- 이정도면 처음 한거 치고 나쁘진 않은듯?
- 일단 돌아가는거 만드는 거 까진 했으니까 초기 목표는 달성하지 않았나...
- C 코드 익숙해지는 연습 따로 하고 다시 만들 생각
    - raylib이랑 터미널로 나눠서 분기처리 가능하게?
